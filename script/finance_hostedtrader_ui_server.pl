...
my $help              = 0;
my $host              = undef;
my $port              = $ENV{FINANCE_HOSTEDTRADER_UI_PORT} || $ENV{CATALYST_PORT} || 3000;
my $keepalive         = 0;
my $restart           = $ENV{FINANCE_HOSTEDTRADER_UI_RELOAD} || $ENV{CATALYST_RELOAD} || 0;
my $background        = 0;
my $pidfile           = undef;
...
my $runner = sub {
    ...
    require Finance::HostedTrader::UI;

    Finance::HostedTrader::UI->run(
        $port, $host,
        {
            argv       => \@argv,
            'fork'     => $fork,
...
finance_hostedtrader_ui_server.pl - Catalyst Testserver

=head1 SYNOPSIS

finance_hostedtrader_ui_server.pl [options]
